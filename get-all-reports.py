...
def try_get_all_applications(attempt=1):
    ...
    try:
        ...
        logger.warning(f"Attempt {attempt}: Failed to get applications - {e}")
...
def try_get_report(build_id, file_format, attempt=1):
    try:
        ...
        logger.warning(f"Attempt {attempt}: Failed to get report for build {build_id} - {e}")
        if attempt < MAX_ATTEMPTS:
            time.sleep(2 ** attempt)
            return try_get_report(build_id, file_format, attempt + 1)
        else:
            logger.error(f"Exceeded maximum retry attempts for build {build_id}. Skipping.")
            return None
...
def save_reports_from_build_list(build_list, base_directory, file_format):
    if not build_list:
        logger.info(f"No builds found in {base_directory}")
        return

    for build in build_list:
        logger.info(f"Requesting report for build {build['build_id']} version {build.get('version')}")
        ...
        if report:
            ...
            with open(file_name, "wb") as file:
                file.write(report)
            logger.info(f"Saved report to {file_name}")
        else:
            logger.warning(f"No report retrieved for build {build['build_id']}")

def try_get_sandbox_list(app_id, attempt=1):
    logger.info(f"Fetching sandbox list for app {app_id}")
    try:
        raw = XMLAPI().get_sandbox_list(app_id)
        return parse_xml(raw)
    except Exception as e:
        logger.warning(f"Attempt {attempt}: Failed to get sandbox list for app {app_id} - {e}")
        if attempt < MAX_ATTEMPTS:
            time.sleep(2 ** attempt)
...
def main():
    ...
    for app in applications:
        ...
        base_directory = os.path.join(out_dir, app_name)
        ...
        for sandbox in sandbox_list:
            ...
            sandbox_directory = os.path.join(base_directory, f"Sandbox {sandbox_name}")
